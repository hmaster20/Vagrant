...
# ---------------------------------
BOX_BASE = "debian/bullseye64" # Debian 11
...
# ---------------------------------
MAC_ADDR = "080027ABC176"
IP_ADDR = "192.168.1.176"      #  Bridge
IP_LOCL = "192.168.50.76"      #  Internal
IP_HOST = "192.168.56.76"      #  Host-Only
# ---------------------------------
HOST_ADDR = "deb-manager.local"
VM_GROUP = "/local"
VM_DESCR = "Debian Server 11 with UI"
# ---------------------------------
...
Vagrant.configure("2") do |config|
    ...
    config.vm.define HOST_ADDR do |machine|
      ...
      # apt-get remove -yqq unattended-upgrades cron-apt 2> /dev/null
      # apt-get purge -yqq unattended-upgrades 2> /dev/null
      # "
      machine.vm.provision "shell", inline: "
      apt-get remove -yqq unattended-upgrades cron-apt
      apt-get purge -yqq unattended-upgrades
      "

      # Remove AppArmor
      machine.vm.provision "shell", inline: "
      apt-get remove --assume-yes --purge apparmor
      "
      ...
      # sudo usermod -aG docker hmaster
      # docker --version
      ...
      # sudo reboot
      # sudo apt install -yq dkms linux-headers-$(uname -r) build-essential
